...
class Swap {
  ...
  static async create(swapData) {
    ...
    const result = await db.query(
      `INSERT INTO swaps(user_requesting, collection_requested, user_offered) VALUES ($1, $2, $3) RETURNING *`,
      [user_requesting, collection_requested, user_offered]
    );
  ...
  async destroy() {
    const result = await db.query(
      `DELETE FROM swaps WHERE swap_id = $1 RETURNING *`,
      [this.swap_id]
    );
  ...
  static async findByUserId(user_id) {
    const result = await db.query(
      `SELECT * FROM swaps WHERE user_requesting = $1 OR user_offered = $1`,
      [user_id]
    );
  ...
  static async findBySwapId(swap_id) {
    const result = await db.query(`SELECT * FROM swaps WHERE swap_id = $1`, [
      swap_id,
    ]);
